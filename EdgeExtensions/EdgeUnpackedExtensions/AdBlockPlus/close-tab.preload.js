...
;// ./src/close-tab/content/close-tab.ts
...
function onClick(event) {
    ...
    if (!element.matches(`[data-extension-action="close-tab"]`)) {
        return;
    }
    event.stopPropagation();
    event.preventDefault();
    void category_app_close();
}
function close_tab_start() {
    document.addEventListener("click", onClick);
}
close_tab_start();

;// ./src/close-tab/content/index.ts
