...
sub constantes()
  ...
  dim baBack% : baBack% = 0
  dim baHome% : baHome% = 1
  dim baSetHome% : baSetHome% = 2
  dim baRefresh% : baRefresh% = 3
end_sub

sub labels()
  label close0, CreerWB, SVT, Back, Home, SetHome, Refresh 
  label shiftHorz, shiftVert, zoomHorz, zoomVert
end_sub
...
sub GUI()           
  ...
  button 3 : top 3,10 : left 3,230 : caption 3,"Retour" : on_click 3,Back
  button 4 : top 4,10 : left 4,left(3)+width(3) : caption 4,"Accueil" : on_click 4,Home
  button 5 : top 5,10 : left 5,left(4)+width(4) : caption 5,"Nouv. Accueil" : on_click 5,SetHome : width 5,100
  button 6 : top 6,10 : left 6,left(5)+width(5) : caption 6,"Recharger" : on_click 6,Refresh
...
CreerWB:
  ...
  res% = dll_call2("SetBrowserNewWindowRequested",WB%,1)
  return
...
SVT:
  ...
  res% = dll_call2("SetBrowserScrollBars",WB%,1)
  return          
  
Back:
  res% = dll_call2("BrowserAction",WB%,baBack%)
  return
  
Home:
  res% = dll_call2("BrowserAction",WB%,baHome%)
  return
  
SetHome:
  res% = dll_call2("BrowserAction",WB%,baSetHome%)
  return
  
Refresh:
  res% = dll_call2("BrowserAction",WB%,baRefresh%)
  return
